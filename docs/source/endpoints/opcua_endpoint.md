...
-   `userMap` is a map with user authentication credentials for the OPC UA Endpoint. The key is the username, the value is the password. If the map is empty, authentication with username and password is not possible. This value is only relevant, when "UserName" is included in `supportedAuthentications`.
