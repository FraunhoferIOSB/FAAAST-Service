...
| -------------------------------------------- | -------------- | ----------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------- |
| aasRegistries<br>*(optional)*                | List<String>   | URLs of AAS registries to use (optionally including /api/{version}). If not set, no synchronization of AASs with registry happens.<br>The endpoint addresses in the descriptors are defined by the "hostname" variable in the EndpointConfigurations of the FA³ST Service (see: [HTTP endpoint configuration](https://faaast-service.readthedocs.io/en/latest/interfaces/endpoint.html#http)) | *empty*                         |
...
| submodelRegistries<br>*(optional)*           | List<String>   | URLs of submodels registries to use (optionally including /api/{version}). If not set, no synchronization of submodels with registry happens.<br>The endpoint addresses in the descriptors are defined by the "hostname" variable in the EndpointConfigurations of the FA³ST Service (see: [HTTP endpoint configuration](https://faaast-service.readthedocs.io/en/latest/interfaces/endpoint.html#http)) | *empty*                         |
