...
public class LogicalBinaryComparisonParser extends AbstractBinaryComparisonParser<LogicalExpression, AbstractBinaryComparison> {
    ...
    public AbstractBinaryComparison parse(LogicalExpression expression) {
        ...
        if (notNullNorEmpty(expression.get$ne())) {
            return (buildComparison(NotEqualsOperation::new, expression.get$ne()));
        }
        if (notNullNorEmpty(expression.get$gt())) {
            return (buildComparison(GreaterThanOperation::new, expression.get$gt()));
        }
        if (notNullNorEmpty(expression.get$ge())) {
            return (buildComparison(GreaterThanEqualsOperation::new, expression.get$ge()));
        }
        if (notNullNorEmpty(expression.get$lt())) {
            return (buildComparison(LessThanOperation::new, expression.get$lt()));
        }
        if (notNullNorEmpty(expression.get$le())) {
            return (buildComparison(LessThanEqualsOperation::new, expression.get$le()));
        }
        return null;
    }
}
