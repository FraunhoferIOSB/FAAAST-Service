...
public class LogicalStringComparisonParser extends AbstractStringComparisonParser<IdtaLogicalExpression, AbstractStringComparison> {
    ...
    public AbstractStringComparison parse(IdtaLogicalExpression expression) {
        ...
        if (notNullNorEmpty(expression.get$startsWith())) {
            return (buildComparison(StartsWithComparison::new, expression.get$startsWith()));
        }
        if (notNullNorEmpty(expression.get$endsWith())) {
            return (buildComparison(EndsWithComparison::new, expression.get$endsWith()));
        }
        if (notNullNorEmpty(expression.get$regex())) {
            return (buildComparison(RegexComparison::new, expression.get$regex()));
        }
        return null;
    }
}
