...
public interface SegmentProvider<T extends Segment, C extends SegmentProviderConfig> extends Configurable<C> {
     ...
     * @param metadata the metadata of the time series
     * @param segment the segment to read from
     * @param start the start time, null represents no restrictions on start of time interval
     * @param end the end time, null represents no restrictions on end of time interval
     * @return all records of the segment within given time interval
     */
    public default List<Record> getRecords(Metadata metadata, T segment, ZonedDateTime start, ZonedDateTime end) {
        return getRecords(metadata, segment).stream()
                .filter(x -> (start == null || !x.getTime().isBefore(start))
                        && (end == null || !x.getTime().isAfter(end)))
