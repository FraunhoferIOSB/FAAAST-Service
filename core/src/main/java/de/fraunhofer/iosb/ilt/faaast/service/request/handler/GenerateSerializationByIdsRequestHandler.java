...
/**
 * Class to handle a {@link de.fraunhofer.iosb.ilt.faaast.service.model.request.GenerateSerializationByIdsRequest} in
 * the service and to send the corresponding response
 * {@link de.fraunhofer.iosb.ilt.faaast.service.model.api.response.GenerateSerializationByIdsResponse}. Is responsible
                ...
                .payload(new DefaultAssetAdministrationShellEnvironment.Builder()
                        .assetAdministrationShells(
                                request.getAasIds().stream()
                                        .map(LambdaExceptionHelper.rethrowFunction(x -> persistence.getOfType(x, OUTPUT_MODIFIER, AssetAdministrationShell.class)))
                                        .collect(Collectors.toList()))
                        .submodels(request.getSubmodelIds().stream()
                                .map(LambdaExceptionHelper.rethrowFunction(x -> persistence.getOfType(x, OUTPUT_MODIFIER, Submodel.class)))
                                .collect(Collectors.toList()))
