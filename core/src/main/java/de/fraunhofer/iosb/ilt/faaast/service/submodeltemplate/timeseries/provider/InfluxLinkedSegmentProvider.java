...
/**
 * Data provider for linked segments referecing an InfluxDB datasource.
 */
public class InfluxLinkedSegmentProvider implements LinkedSegmentProvider<InfluxLinkedSegmentProviderConfig> {
    ...
    public List<Record> getRecords(Metadata metadata, LinkedSegment segment) {
        InfluxDBClient client = InfluxDBClientFactory.create(segment.getEndpoint(), config.getUsername(), config.getPassword().toCharArray());
        List<FluxTable> influxRecords = client.getQueryApi().query(segment.getQuery());
        // convert to List<Record>
        return null;
    }
